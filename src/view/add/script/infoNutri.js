...
function createInputBoxInfoNutri(index, divId) {
...
function createSelectListInfoNutri(index, divId) {
    ...
    for(i=0; i<infos.length; i++) {
        ...
        if(infos[i] === optionName) checkItem.setAttribute("selected", "selected");
        selectList.appendChild(checkItem);
    }
    indisponiveis[optionName] = true;
    valorAnt[selectList.id] = optionName;
...
function addInfoNutri(divId) {
    ...
    let selectList = createSelectListInfoNutri(index, divId);
    let inputBox = createInputBoxInfoNutri(index, divId);
